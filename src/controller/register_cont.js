...
const MAIL_KEY =
  "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";
  ...
  return new Promise(resolve => {
    ...
    const msg = {
      ...
      text:
        ...
        "://" +
        req.headers.host +
        "/user/verify?confirmation=" +
        token
    };

    sgMail.send(msg, (err, result) => {
      if (err) resolve({ send_error: 1 });
...
exports.mail = async (req, res) => {
  ...
  jwt.sign({ url: url }, PRIVATE_KEY, async (err, token) => {
    ...
    if (result.send_error)
      res
        .status(500)
        .json(resFun.fail(500, "An error occured while sending mail"));

    res
      .status(200)
      .json(resFun.success(200, "The mail was sent in successfully", null));
  });
};
...
exports.register = async (req, res) => {
  ...
  if (!hash_error)
    ...
    res
      .status(500)
      .json(resFun.fail(500, "An error occured while hashing password"));
    return;
  }
  ...
  if (!insertErr)
    res
      .status(200)
      .json(resFun.success(200, "Registered in successfully", null));
